...
线程虽然能并发，但线程的创建与切换本身也有代价（8MB的栈，上下文切换），创建和切换线程的代价有可能比函数调用还要大。
...
这看起来完全是同步阻塞代码，但Go的运行时不会让这个goroutine在`read`处阻塞，而是让它睡去，等有消息了再唤醒，背后和`epoll`那套是一样的。
...
此时有一种每个语言的并发模型区别很大的感觉，学得会有点累。

着重点要关注四点，所有并发模型都在回答这四个问题：

- 并发单元是什么——线程?协程?进程?回调?

- 谁来调度——内核?用户态运行时?事件循环?

- 它们怎么通信——共享内存+锁?消息传递?异步返回值?

- 等待时谁被阻塞——阻塞整个内核线程?还是只挂起一个轻量单元?

## 实验

根据骨架代码实现一个http服务器，支持并发。

CGI协议是做这个实验才知道的，Common Gateway Interface（通用网关接口），规定HTTP服务器怎么调用外部程序。

``` txt
GET /cgi-bin/hello.py?name=wang HTTP/1.1
```

如果url以`cgi-bin`开头，服务器应该启动对应的程序，并把参数通过环境变量和标准输入传进去。程序的输出再包装成http response返回。

本质是每个请求fork一个新进程：`请求进来 → fork + exec 一个解释器跑脚本 → 脚本 stdout 变成响应 → 进程销毁`。

不过CGI已经过时了，现代的开发方法至少是像Go net/http包那样，服务器和应用程序是同一个进程，或者像Nginx那样转发请求，而不是fork新进程。